...
// GetContext method returns the GetContext if its already set in request
...
func (r *Request) GetScheme() string {
	return r.RawRequest.URL.Scheme
}
...
func (r *Request) GetHostName() string {
	return r.RawRequest.URL.Hostname()
}

func (r *Request) GetPort() string {
	return r.RawRequest.URL.Port()
}
...
func (r *Request) GetRaw() ([]byte, error) {
	...
	r.raw = append(reqHeaderRaw, body...)
	return r.raw, nil
}
...
// SetContext method sets the context.GetContext for current Request. It allows
...
// SetIndex 设置自动推送请求响应事件的index
func (r *Request) SetIndex(i interface{}) *Request {
	r.index = i
	return r
}
...
func (r *Request) GetLocalAddrBySelf() *Request {
	r.GetLocalAddrSelf = true
	return r
}

// GetLocalAddrByOS 由操作系统自己选择连接目标的地址
func (r *Request) GetLocalAddrByOS() *Request {
	r.GetLocalAddrSelf = false
	return r
}
...
// ExpectContentType 用于设置自动反序列化返回包内容使用的方式（xml和json）优先级小于返回包中服务器设置的
func (r *Request) ExpectContentType(contentType string) *Request {
	r.fallbackContentType = contentType
	return r
}

// ForceContentType 用于设置自动反序列化返回包内容使用的方式（xml和json）优先级大于返回包中服务器设置的
...
func (r *Request) ConnectInfo() ConnectInfo {
	...
	if ct == nil {
		ct = &clientTrace{
			getConn:     r.Time,
			connectDone: r.RequestDoneTime,
		}
	}
	ci = ConnectInfo{
		GetConnectTime:          ct.getConn,
		GotConnectTime:          ct.gotConn,
		ConnectDone:             ct.connectDone,
		ReceiveHttpResponseTime: ct.gotFirstResponseByte,
	}
	if ct.gotConnInfo.Conn != nil {
		ci.RemoteAddr = ct.gotConnInfo.Conn.RemoteAddr()
	...
	if ci.GetConnectTime.IsZero() {
		ci.GetConnectTime = r.Time
	}
	if ci.ConnectDone.IsZero() {
		ci.ConnectDone = r.RequestDoneTime
	}
	return ci
}
