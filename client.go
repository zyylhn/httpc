...
var (
	...
	hdrUserAgentValue = "Mozilla/5.0 (Windows NT 10.0; rv:78.0) Gecko/20100101 Firefox/78.0"
...
type Client struct {
	...
	PushTraceEvent        []PushTraceEvent //推送请求响应事件接口
	...
	GetLocalAddrSelf       bool   //连接的时候是否使用自己获取的地址
	LocalIp                string //使用的本地地址

	usingPort           map[int]struct{}
	usingPortLock       sync.RWMutex
	jsonEscapeHTML      bool
...
// Also it can be overridden at request level header options,在request中设置的会覆盖在这里面设置的
//
...
// AppendPushEvent 新增推送接口
func (c *Client) AppendPushEvent(push ...PushTraceEvent) *Client {
	c.PushTraceEvent = append(c.PushTraceEvent, push...)
	return c
}
...
func (c *Client) SetRedirectPolicy(policies ...RedirectPolicy) *Client {
	return c.setRedirectPolicy(policies...)
}
...
// DisableRedirect 禁用302跳转
func (c *Client) DisableRedirect() *Client {
	return c.setRedirectPolicy(NoRedirectPolicy())
}

// EnableRedirectNum 跳转指定次数
func (c *Client) EnableRedirectNum(n int) *Client {
	c.RedirectCount = n
	return c.setRedirectPolicy(FlexibleRedirectPolicy(n))
}

// EnableRedirect 启用跳转
func (c *Client) EnableRedirect() *Client {
	if c.RedirectCount < 0 {
		return c.EnableRedirectNum(10)
	} else {
		return c.EnableRedirectNum(c.RedirectCount)
	}
}

func (c *Client) DisableKeepAlive() *Client {
	c.httpClient.Transport.(*http.Transport).DisableKeepAlives = true
	return c
}

func (c *Client) EnableHTTP2() error {
	err := http2.ConfigureTransport(c.httpClient.Transport.(*http.Transport))
	if err != nil {
		return err
	}
	return nil
...
// EnableRedirectWithHost 仅跳转到指定host的时候允许跳转
func (c *Client) EnableRedirectWithHost(hostname ...string) *Client {
	return c.setRedirectPolicy(DomainCheckRedirectPolicy(hostname...))
}
...
func (c *Client) SkipTLSVerify() *Client {
	return c.SetTLSClientConfig(&tls.Config{InsecureSkipVerify: true})
}
...
// GetLocalAddrBySelf 由我们自己去获取连接远程目标的地址
func (c *Client) GetLocalAddrBySelf(localIP string) *Client {
	c.GetLocalAddrSelf = true
	c.LocalIp = localIP
	return c
}

// GetLocalAddrByOS 由操作系统自己选择连接目标的地址
func (c *Client) GetLocalAddrByOS() *Client {
	c.GetLocalAddrSelf = false
	return c
}
...
func (c *Client) execute(req *Request) (*Response, error) {
	...
	if req.LocalAddr != nil {
		...
		newClient.Jar = c.httpClient.Jar
		newClient.CheckRedirect = c.httpClient.CheckRedirect
		newClient.Timeout = c.httpClient.Timeout
		newTransport := c.httpClient.Transport.(*http.Transport).Clone()
	R:
		...
		resp, err = newClient.Do(req.RawRequest)
		c.usingPortLock.Lock()
		delete(c.usingPort, req.LocalAddr.(*net.TCPAddr).Port)
...
func (c *Client) onErrorHooks(req *Request, resp *Response, err error) {
	...
	for _, f := range c.PushTraceEvent {
		errString := ""
		if err != nil {
			errString = err.Error()
		}
		f.Push(req.index, NewTraceEvent(req, resp, errString))
	}
}
