...
func TestPostXMLStructInvalidLogin(t *testing.T) {
	...
	assertError(t, err)
	assertEqual(t, http.StatusUnauthorized, resp.StatusCode())
	assertEqual(t, resp.GetHeaders().Get("Www-Authenticate"), "Protected Realm")
...
func TestOptionsMethod(t *testing.T) {
	...
	assertError(t, err)
	assertEqual(t, http.StatusOK, resp.StatusCode())
	assertEqual(t, resp.GetHeaders().Get("Access-Control-Expose-Headers"), "x-go-resty-id")
}
...
func TestSendMethod(t *testing.T) {
	...
	t.Run("send-options", func(t *testing.T) {
		...
		assertEqual(t, "", resp.String())
		assertEqual(t, "x-go-resty-id", resp.GetHeaders().Get("Access-Control-Expose-Headers"))
	})
...
func TestRequestGetRaw(t *testing.T) {
	c := NewDefaultClient()
	req := c.NewRequest().SetBody("asdasdasdasd")
	fmt.Println(req.GetRaw())
	_, err := req.Get("http://127.0.0.1:700")
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println(req.GetRaw())
