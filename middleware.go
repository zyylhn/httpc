...
func createHTTPRequest(c *Client, r *Request) (err error) {
	if r.bodyBuf == nil {
		...
		//todo 将bodyBuf中的内容读取出来保存到raw中
		data, newErr := ioutil.ReadAll(bodyBuf)
		if newErr == nil {
			r.BodyRaw = data
			newBuf := bytes.NewBuffer(data)
			r.RawRequest, err = http.NewRequest(r.Method, r.URL, newBuf)
		} else {
			err = newErr
		}
	}
	...
	if c.trace || r.trace {
		...
		r.ctx = r.clientTrace.createContext(r.GetContext())
	}
...
func addCredentials(c *Client, r *Request) error {
	...
	// Set the Authorization GetHeaders Scheme
...
func responseLogger(c *Client, res *Response) error {
	if c.Debug {
		rl := &ResponseLog{Header: copyHeaders(res.GetHeaders()), Body: res.fmtBodyString(c.debugBodySizeLimit)}
...
func parseResponseBody(c *Client, res *Response) (err error) {
	...
	ct := firstNonEmpty(res.Request.forceContentType, res.GetHeaders().Get(hdrContentTypeKey), res.Request.fallbackContentType)
